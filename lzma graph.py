...
import lzma  # Import lzma for LZMA compression
import os
...
# Function to compress image using LZMA compression algorithm
def compress_image_to_file(file_path):
    compressed_lzma_path = 'compressed_image.lzma'
    ...
    with open(compressed_lzma_path, 'wb') as compressed_file:
        compressed_file.write(compressed_data)

    original_size = len(original_data)
    compressed_lzma_size = len(compressed_data)
    compression_ratio = original_size / compressed_lzma_size
    ...
    return compressed_lzma_path, original_size, compressed_lzma_size, image_format, image_size, image_resolution, image_quality
...
def display_compression_details(original_size, compressed_lzma_size, image_format, image_size, image_resolution, image_quality):
    original_size_str = convert_size(original_size)
    compressed_lzma_size_str = convert_size(compressed_lzma_size)

    compression_ratio = original_size / compressed_lzma_size
    bandwidth_saved = (1 - compression_ratio) * 100

    print(f"Compression results (LZMA):\n"
          f"Original file size: {original_size_str}\n"
          f"Compressed file size: {compressed_lzma_size_str}\n"
...
def main():
    ...
    if file_path:
        lzma_path, original_size, compressed_lzma_size, image_format, image_size, image_resolution, image_quality = compress_image_to_file(file_path)
        display_compression_details(original_size, compressed_lzma_size, image_format, image_size, image_resolution, image_quality)
